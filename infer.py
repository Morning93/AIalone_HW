...
from utils.HWparser import parse_infer_args, parse_train_args
from utils.utils import get_ckpt_path, get_hparam_path
from utils.get_modules import prepare_image, get_model, get_transform
'''
import os 
...
def main():
    ...
    # 전처리 과정 등에서 학습 당시의 하이퍼 파라미터들이 필요하므로 소환!!
    ckpt_path = get_ckpt_path(args)
    hparam_path = get_hparam_path(args)
    ...
    transform = Compose([Resize((train_args.image_size, 
    ...
    '''
    model = myMLP(image_size=train_args.image_size, 
                hidden_size=train_args.hidden_size, 
                num_class=train_args.num_class).to(args.device) 
    '''
